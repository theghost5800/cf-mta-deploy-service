...
class BindServiceStepTest extends SyncFlowableStepTest<BindServiceStep> {
    ...
    void testDoNotThrowExceptionOnOptionalService() {
        ...
        customProcessContext.setVariable(Variables.SERVICES_TO_BIND, List.of(ImmutableCloudServiceInstanceExtended.builder()
                                                                                                                  ...
                                                                                                                  .isOptional(true)
                                                                                                                  .build()));
        assertDoesNotThrow(() -> new BindServiceStep.DefaultApplicationServicesUpdateCallback(customProcessContext).onError(new CloudOperationException(HttpStatus.BAD_GATEWAY),
                                                                                                                            APPLICATION_NAME,
                                                                                                                            SERVICE_NAME));
    }
    ...
    void testThrowExceptionOnNotOptionalService() {
        ...
        assertThrows(SLException.class,
                     () -> new BindServiceStep.DefaultApplicationServicesUpdateCallback(customProcessContext).onError(new CloudOperationException(HttpStatus.BAD_GATEWAY),
                                                                                                                      APPLICATION_NAME,
                                                                                                                      SERVICE_NAME));
    }

    @Override
    protected BindServiceStep createStep() {
        return new BindServiceStep();
    }
